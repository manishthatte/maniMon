...
--status)
    ...
    python3 "$HERE/metrics.py" --info 2>/dev/null | grep -E '"rows"|"size_mb"'
    exit 0
    ;;
--remove)
...
for u in "${UNITS[@]}"; do
    ...
    sed "s|@INSTALL_DIR@|$HERE|g" "$SRC/$u" > "$DEST/$u" \
...
echo "  statistics:    python3 $HERE/metrics.py --report"
echo "  per-run costs: python3 $HERE/runs.py"
echo "  configuration: python3 $HERE/config.py"
