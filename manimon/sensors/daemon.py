...
from ..config import SENSOR_DIR as OUT_DIR, CFG
...
# Resolved, not hardcoded: these live in sbin on some distributions and bin on
# others. A missing tool is not an error here — the sampler that needs it
# reports itself unavailable and the rest carry on.
IPMITOOL  = which('ipmitool')  or '/usr/bin/ipmitool'
NVME      = which('nvme')      or '/usr/sbin/nvme'
SMARTCTL  = which('smartctl')  or '/usr/sbin/smartctl'
DMIDECODE = which('dmidecode') or '/usr/sbin/dmidecode' 
...
def main(argv=None):
    ...
    if os.geteuid() != 0:
        print("sensord must run as root (it reads /dev/ipmi0 and raw disks).",
              file=sys.stderr)
        print("  sudo manimon sensors --once --print", file=sys.stderr)
        return 2
